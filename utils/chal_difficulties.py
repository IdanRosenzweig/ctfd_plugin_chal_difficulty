...
def sync():
    chals_ids = [
        chal.id
        for chal in Challenges.query.order_by(Challenges.category, Challenges.id).all()
    ]

    # remove any invalid records
    records = {d.challenge_id: [False, d] for d in database.get_all()}

    for id, _data in records.items():
        if id in chals_ids:
            records[id][0] = True

    for _id, data in records.items():
        if not data[0]:
...
def set(request_form):
    ...
    for key, value_str in request_form.items():
        if key.startswith("chal_"):
            ...
            if (
                value_str == "0" or value_str == ""
            ):  # if set to 0 or invalid, delete the record to keep the database clean
                if record:
