...
def add(challenge_id, difficulty) -> None:
    new_chal_difficulty = ChalDifficulties(
        challenge_id=challenge_id, difficulty=difficulty
    )
...
def get_by_chal_id(chal_id):
    return ChalDifficulties.query.filter_by(challenge_id=chal_id).first()
